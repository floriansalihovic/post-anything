...
from tests.fixtures import (
    PROP_NAME_RESOURCE_PATH,
    get_default_document_filter,
    get_default_json,
)
...
def test_document_api(new_mongo_container: MongoDbContainer):
    document_filter = get_default_document_filter()
...
def test_http_api(new_mongo_container: MongoDbContainer):
    resource_path = f"/{get_default_document_filter().get(PROP_NAME_RESOURCE_PATH)}"
    ...
    response = test_client.post(resource_path, json=default_json)
    ...
    response = test_client.put(resource_path, json=default_json)
    assert HTTP_200_OK == response.status_code
    assert PROP_NAME_RESOURCE_PATH in response.json()
